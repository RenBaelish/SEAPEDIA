                ...
                {cart.items.map(item => (
                  <div key={item.id} className="flex gap-4 pb-4 border-b-2 border-gray-100 last:border-0 last:pb-0">
                    <div className="w-16 h-16 border-2 border-nb-black bg-gray-50 overflow-hidden shrink-0">
                      <img src={item.product.images?.[0] ? (typeof item.product.images[0] === 'string' ? item.product.images[0] : item.product.images[0]?.url) : "https://placehold.co/400x400/F7F5F0/0A0A0A?text=No+Image"} alt={item.product.name} className="w-full h-full object-cover" />
                    </div>
